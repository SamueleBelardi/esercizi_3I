...
public class ScambioVettori extends Application {
	...
	private void eseguiScambia() {
		...
		for (int i = 0; i < numeriUno.length; i++) {
			numeriDue[contatore] = numeriUno[i];
			contatore--;
		}
		for (int i = 0; i < numeriDue.length; i++) {
			if(risultato == "") {
				risultato+= " " + numeriDue[i];
			} else {
				risultato+= ", " + numeriDue[i];
			}		
		}
		cRisultato.setText(risultato);
	}
